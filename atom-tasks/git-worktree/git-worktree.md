...
3. 使用 runtime 注入的 `{{runtime.runType}}` 作为分支前缀（仅可能是 feat 或 fix）。生成分支名并记录 type 与 dateDescription：
...
5. 如果 reuseExisting 为 true，检查是否已存在该分支的工作树（通过 git worktree list）。如果找到，复用它并继续执行 runtime attach。
6. 否则，创建分支和工作树：
...
9. 生成 worktree-info JSON，调用 runtime `attach-worktree`，传入 runId、worktreePath、dateDescription、artifactDir 与 worktree-info。runtime 负责设置最终锚点、刷写 pending outputs、登记 worktree-info 并返回新的 statePath；attach 失败时保留 bootstrap state 并停止。
10. 切换 agent 工作目录：
    a. 使用 runtime 注入的 `{{runtime.projectRoot}}`，并保持 skillRoot 只读。
...
- worktree 接入必须通过 `attach-worktree`；任务不得直接编辑 `.state.json`。后续命令使用 runtime 返回的新 statePath。
- 不得在主工作树中执行任何文件修改操作。

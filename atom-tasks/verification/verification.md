...
如果所有自动检查和已确认的人工检查都通过，追加最终行 `ALL PASSED`。如果存在失败，生成 verification-log 后调用 runtime `request-retry --from verification --target coding --reason <原因>`；不得直接重写 stage、history 或 `currentStage`，也不得写 `ALL PASSED`。最多自动修复 `options.maxRetries` 轮（默认 2），轮次通过 `record-node-progress --node verification --key retryCount` 记录。达到上限时调用 `record-node-failure` 并停止。若仅存在尚未确认的人工检查，调用 `mark-node-waiting-human --node verification` 并暂停。
...
- 如果当前是 issue-driven 工作流：
  ...
  ```
- 暂停，等待人工介入
- 通过 runtime `record-node-progress` 记录重试轮次

## 约束

- 不得执行 `human:` 行；仅展示。
- 在 `{{runtime.worktreePath}}` 工作目录中执行 `cmd:` 行；不得在 worktreeDir、主工作树或其他外部路径执行修改性命令。

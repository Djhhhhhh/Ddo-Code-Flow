...
| # | 问题 | 状态 |
|---|---|---|
| O1 | ~~相位数与相位推进的数据来源（`spec:01→:02` 靠什么驱动）~~ | **已关闭（05）**：原子任务 `config.phases` 自声明 |
| O2 | ~~`waiting-human` 等状态的进入/离开归属（next 扩展 or 门语义命令）~~ | **已关闭（07）**：进入 = next/start 置 `waiting-human` 并写门（`stages[k].gate`）；离开 = `next --decision <name>`（推进型决议，CLI 拦截未决议推进） |
| O3 | ~~`stages ↔ 原子任务` 映射（next/exec 需要「该阶段执行哪个任务」）~~ | **已关闭（06）**：stageId 即任务名 |
| O4 | ~~`_del` 归档位置（对「项目工作目录」的解释）~~ | **已关闭（11，12 轮确认）**：定版 runDir（= run 工作目录 = 流水线产物目录，`state.dirs.runDir` 显式化） |

...
- 让 config.json 成为工作流索引、选择规则和运行时配置的唯一事实来源。
- 让 skill 执行时根据配置和 .state.json 渐进式读取所需文件，避免一次性加载全部上下文。
- 让 UI 能在 panel__head 中切换并预览不同模式下的工作流。
...
| config.json | skill 加载逻辑和 UI 读取配置时使用的唯一事实来源。 |
| .state.json | 单次 run 的状态文件，用于决定当前阶段和下一步需要渐进式读取的文件。 |
...
- **FR-WF-1**：系统应支持通过 workflows 目录维护多个 workflow 定义。
...
### 3.2 config.json 职责重定义

- **FR-CONFIG-1**：config.json 应作为唯一事实来源，描述全局运行配置、workflow 索引、默认 workflow、分类规则和 atom-task override 入口。
- **FR-CONFIG-2**：config.json 应能索引到具体 workflow 定义，运行时不得依赖写死的单一 pipeline。
- **FR-CONFIG-3**：配置结构应支持校验，避免 workflow 引用缺失、DAG 结构错误或默认 workflow 不存在。
- **FR-CONFIG-4**：系统应保留已有配置可迁移或兼容的路径，降低现有用户升级成本。

### 3.3 skill 加载逻辑

- **FR-RUNTIME-1**：skill 启动后应先读取 config.json 和 schema，并基于配置解析目标 workflow。
- **FR-RUNTIME-2**：skill 应根据 .state.json 判断当前 run 是否需要恢复，以及恢复后应从哪个 stage/node 继续。
...
- **FR-UI-1**：Studio UI 应在中间 topology 面板的 panel__head 区域新增 workflow 切换控件。
- **FR-UI-2**：用户切换 workflow 后，UI 应预览该模式下的 stage、atom-task 节点和 DAG 连线。
- **FR-UI-3**：UI 应显示当前 workflow 的基础信息，帮助用户理解不同模式的用途。
- **FR-UI-4**：UI 的保存逻辑应保持 config.json 与 workflow 定义一致，避免只更新预览状态而未持久化配置。
...
- **C-4**：渐进式加载不得破坏现有 .state.json 恢复能力。
...
- **AC-1**：仓库中存在 3 个可被 config.json 索引的 workflow 定义。
- **AC-2**：3 个 workflow 均只引用现有 atom-task。
- **AC-3**：config.json 不再承担单一 pipeline 的唯一承载职责，而是能索引到具体 workflow。
- **AC-4**：skill 文档明确以 config.json 为唯一事实来源，并描述 workflow 解析、参数覆盖、规则匹配和默认回退逻辑。
- **AC-5**：skill 文档明确根据 .state.json 和当前 node 渐进式读取 atom-task 文件、schema 和输入文件。
- **AC-6**：Studio UI 的 panel__head 区域提供 workflow 切换控件。
...
- **Q-2**：config.json 与 workflows/*.json 的字段边界如何划分，才能同时满足唯一事实来源和可维护性？——留给 plan.md。
- **Q-3**：旧版 config.json 中已有 pipeline 的兼容策略是迁移、运行时兼容，还是二者都支持？——留给 plan.md。

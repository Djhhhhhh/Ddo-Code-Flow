...
- [ ] plan.md:75（算法设计③）— 跨列正交通道路由为预留实现：当前 state 数据无跨列边未触发，未来加入回退/转移留痕可视化时启用（TODO 性质的预留项，已在 plan 中声明）
- [ ] visualize.js:104-125（countCrossings）— 自检计数器已实现，但对抗样例集暂未包含跨列边用例，路由路径的防交叉尚无实证

> 注：TODO/FIXME/XXX 字面标记于 visualize.js 零命中；以上为 plan 声明的预留项与对应验证缺口，非字面标记。
...
> 注：未完结项扫描结果——TODO/FIXME/XXX 于 visualize.js 零命中（详见本节留空与上方 follow-ups）。
...
- validate 失败后推进命令不被结构拦截（本 run 实测：同批次 validate exit 1 后 next 仍开门）——「不带错推进」目前仅是协议约束，agent 纪律是唯一防线；本次已用 rollback 重做恢复协议完整性
